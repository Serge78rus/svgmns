...
<xsl:stylesheet version='1.0'
	...
	<xsl:template name="valve_b_sym">
		<symbol id="valve_b" viewBox="0 0 1 1" preserveAspectRatio="xMidYMid meet">
	    <rect x=".25" y=".25" width=".5" height=".5" fill="none" stroke-width=".5" />
			<polygon points=".1,.1 .1,.9 .5,.5" stroke="black" stroke-width=".05" />
			<polygon points=".9,.1 .9,.9 .5,.5" stroke="black" stroke-width=".05" fill="black" />
		</symbol>
	</xsl:template>

	<xsl:template match="valve_b" mode="dyn">
		<xsl:param name="i"></xsl:param>
		<xsl:param name="x">0</xsl:param>
		<xsl:param name="y">0</xsl:param>
		<use class="valve_b">
			<xsl:attribute name="xlink:href">
       	<xsl:text>#valve_b</xsl:text>
 	  	</xsl:attribute>
